...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        TreeMap<T3Action, T3State> result = new TreeMap<T3Action, T3State>();
        
        T3Action newAction = null;        
        for (int col = 0; col <= 2; col++) {
            for (int row = 0; row <= 2; row++) {
        	    for (int move : getMoves()) {
                    newAction = new T3Action(col, row, move);
                    if (isValidAction(newAction)) {
                        result.put(newAction, getNextState(newAction));
                    }
        	    }
            }
        }
